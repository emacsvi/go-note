...
// 常规的方式，没有其他依赖，所以直接成为了log.Logger的初始化函数
...
func NewMux(lc fx.Lifecycle, logger *log.Logger) *http.ServeMux {
    ...
    // then exits.
    //
    // 利用fx.Lifecycle添加application Run和Stop的Hook接口。只有在app.Run,Start,Stop的时候才会运行钩子函数。
...
// below for details.
// 在fx.Invoke(Register)的时候进行初始化调用的。
