---
title: "go-supervisord守护进程"
slug: "go-supervisord"
date: "2018-11-23 01:00:28"
categories:
    - go
tags:
    - go
...
以下代码来自template_config.go里面的模块。目前支持的功能都在模板里面定义了。
...
#### directory
当运行一个程序不在当前目录时，可以利用`directory`将程序指向不同的运行目录。

例如下面的`explorerweb`程序就不在`here`运行目录，由**supervisord**程序在运行将它的`Dir`进行修改再运行的。
```ini
[supervisord]
loglevel=debug
...
autorestart=true
startsecs=8
environment=http_proxy="http://10.0.48.51:1087",https_proxy="http://10.0.48.51:1087",LOTUS_PATH="/home/liwei/.lotusnew"
command=/home/liwei/bin/lotus daemon --api 1235
```

## 使用

在我的程序脚本里面经常会用到启动，停止，重启，重新加载等功能：

```bash
echo "启动daemon....."
/Users/liwei/coding/go/filecoin/action/supervisord -c  /Users/liwei/coding/go/filecoin/action/supervisor.conf -d
/Users/liwei/coding/go/filecoin/action/supervisord ctl start lotus
/Users/liwei/coding/go/filecoin/action/supervisord ctl stop miner
```
...
./dminer ctl -s http://127.0.0.1:9029 stop dminer

- [supervisord github](https://github.com/ochinchina/supervisord)
- [所有功能都实现了](https://blog.csdn.net/feng98ren/article/details/79868082)
